...
def metade(valor=0, retorno=True, format=True):
    res = valor / 2
    if retorno:
...
def dobro(valor=0, retorno=True, format=True):
    res = valor * 2
    if retorno:
        return res if format is False else moeda(res)
    else:
...
def aumentar(porcent=0, valor=0, retorno=True, format=True):
...
def diminuir(porcent=0, valor=0, retorno=True, format=True):  # Format diz se deve ou não formatar monetariamente
    res = valor - ((valor / 100) * porcent)
    if retorno:
        return res if format is False else moeda(res)
    else:
...
def resumo(val=0, aumento=0, reducao=0):
    print('-'*30)
    print(f'{"RESUMO DO VALOR":^25}')
    print('-'*30)
    print(f'{"Preço analisado:":<20} {moeda(val)}')
    print(f'{"Dobro do preço:":<20} {dobro(val, format=True)}')
    print(f'{"Metade do preço:":<20} {metade(val, format=True)}')
    print(f'{f"{aumento}% de aumento:":<20} {aumentar(aumento, val, format=True)}')
    print(f'{f"{reducao}% de redução:":<20} {diminuir(reducao, val, format=True)}')
    print('-'*30)
